...
- Each `EnemyUnitData` authors average STR/DEX/CON (`stats`) and `show_held_weapon`. Instance stats roll ±1 via `make_stats()`; enemies do **not** use `PowerTier`. Attack interval is authored on `combat`, not a Stat. Projectiles always come from `combat.projectile_scene` even when held weapon art is hidden.
- Scout UI lives in `scout_bubble/scout_bubble.tscn` (`ScoutBubble`); it fills `GameState.upcoming_enemy_formation`. Roster build reads that array (combat via `BattleLaunch`). Scout reroll costs `BiomassData.reroll_price` (2 biomass + 1 per previous Scout reroll this Day, independent of Shop) and bias-picks a different difficulty (scored from authored average stats) — **disabled on elite days**. Scout shows the formation's **Battle reward** (day base × difficulty), not per-kill sums.
...
- Days 1–2 are Solar Sword-only (`min_day` gates other types); day 3 unlocks Rose Thorn/Peashooter/Stump; day 4+ can include Solar Cleaver/Durian/Log/Canopy/Seed Lobber/Acorn Knight. Day curve scales unit count only. Run start: blocking seal pick (`SealChoiceDialog`, 3 rolled offers), then War Chamber package pick (`StarterPackages` / `StarterChoiceDialog`) — one dual-trained combo Adult (common tier) plus a hidden untrained Child. Starting biomass is 3. Mid-run seal picks still queue after days 2 / 5 / 8 (reroll allowed at `BiomassData.seal_reroll_price`; opening pick cannot reroll).

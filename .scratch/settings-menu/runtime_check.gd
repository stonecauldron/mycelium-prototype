...
func _run() -> void:
	...
	var starter_confirm := starter.get_node("%ConfirmButton") as Button
	_check(not starter_confirm.disabled, "Selecting a starter enables confirmation")
	var starter_cues: Dictionary = Audio.get("_last_ui_cues")
	starter_cues.clear()
	starter_confirm.pressed.emit()
	_check(starter_cues.size() == 1 and starter_cues.has(Sfx.Cue.SEAL), "Starter confirmation plays only the Seal confirmation cue")
	_check(GameState.troop.is_seeded(), "Sound confirmation still grants the starter units")
	await _wait()
	await _capture("base")
	await _key(KEY_ESCAPE)
	await _button(menu, "TitleButton")

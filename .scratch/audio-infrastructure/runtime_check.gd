...
func _check_music() -> void:
	...
	if audio == null or not audio.has_method("play_base_music"):
		return
	audio.set("base_music", _tone(0.2))
	...
	await _wait(0.12)
	_check(base.playing and battle.playing and base.volume_linear > 0.0 and battle.volume_linear > 0.0, "Both tracks play during a crossfade")
	await _wait(0.55)
	_check(not base.playing and battle.playing and is_equal_approx(battle.volume_linear, 1.0), "Crossfade completes in real time during pause and hitstop")
	...
	for i in 8:
		...
		await _wait(0.02)
	await _wait(0.6)
	_check(battle.playing and not base.playing, "Interrupted changes settle on the last requested track")
	audio.set("base_music", null)
	audio.call("play_base_music")
	await _wait(0.6)
	_check(not base.playing and not battle.playing, "Empty track slot fades to silence")
	get_tree().paused = false
	Engine.time_scale = 1.0
...
func _check_scenes_and_menu() -> void:
	...
	_check(base_music.playing and not battle_music.playing, "Title selects Base music")
	...
	_check(battle_music.playing and not base_music.playing, "Starting a Battle selects Battle music")
	...
	for path in ["day_summary/day_summary", "victory/victory", "game_over/game_over"]:
		...
		_check(battle_music.playing and battle_music.get_playback_position() > position, path + " continues Battle music")
	await _scene("res://assets/base/base.tscn")
	_check(base_music.playing and not battle_music.playing, "Returning to Base selects Base music")

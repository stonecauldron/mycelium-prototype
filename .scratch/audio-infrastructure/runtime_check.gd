...
func _check_music() -> void:
	...
	_check(battle.playing and is_equal_approx(battle.volume_linear, 0.2) and battle.get_playback_position() > before, "Battle track continues at 20 percent after returning to Base")
	before = battle.get_playback_position()
	var returned_battle: AudioStreamPlayer = audio.call("play_battle_music")
	await _wait(0.6)
	_check(returned_battle == battle and battle.get_playback_position() > before, "Next Battle restores its existing music without restarting")
	for i in 8:
	...
	await _wait(0.6)
	_check(base.playing and battle.playing and battle.get_playback_position() > before and is_equal_approx(battle.volume_linear, 0.2), "Interrupting a stop restores the mix without resetting either track")
	audio.set("base_music", null)
	audio.call("play_base_music")
	await _wait(0.6)
	_check(not base.playing and battle.playing and is_equal_approx(battle.volume_linear, 0.2), "Empty Base slot stays silent while background Battle music continues")
	audio.call("stop_music")
	await _wait(0.6)
...
func _check_scenes_and_menu() -> void:
	...
	_check(base_music.playing and is_equal_approx(base_music.volume_linear, 1.0) and battle_music.playing and is_equal_approx(battle_music.volume_linear, 0.2), "Title brings Base music forward and lowers Battle music")
	...
	await _scene("res://assets/combat/combat_sandbox/combat_sandbox.tscn")
	...
	_check(base_music.playing and is_equal_approx(base_music.volume_linear, 1.0) and battle_music.playing and is_equal_approx(battle_music.volume_linear, 0.2), "Returning to Base restores the Base mix")

...
- Scout UI lives in `scout_bubble/scout_bubble.tscn` (`ScoutBubble`); it fills `GameState.upcoming_enemy_formation`. Roster build reads that array (combat via `BattleLaunch`). Scout reroll costs `BiomassData.reroll_price` (upcoming Day + extra rerolls this Day) and bias-picks a different difficulty (scored from authored average stats) — **disabled on elite days**. Scout shows the formation's **Battle reward** (day base × difficulty), not per-kill sums.

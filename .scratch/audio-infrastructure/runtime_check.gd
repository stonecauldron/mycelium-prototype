...
func _check_empty_music_start() -> void:
	...
	await _wait(0.65)
	_check(is_equal_approx(battle.volume_linear, 1.0), "First audible music after an empty slot uses the initial half-second fade")
	Audio.play_battle_music(true)
	await _wait(1.0)
	_check(battle.volume_linear > 0.4 and battle.volume_linear < 0.6, "A rematch still uses two seconds when Base has never played")
	Audio.stop_music()
	await _wait(0.6)
...
func _check_music() -> void:
	...
	var battle := Audio.play_battle_music(true)
	await _wait(1.0)
	_check(base.volume_linear > 0.4 and base.volume_linear < 0.6 and battle.volume_linear > 0.4 and battle.volume_linear < 0.6, "Base-to-Battle crossfade is halfway through after one real-time second")
	await _wait(1.15)
	_check(base.stream_paused and is_zero_approx(base.volume_linear) and is_equal_approx(battle.volume_linear, 1.0), "After two seconds only Battle is audible and Base is paused")
	...
	await _wait(4.0)
	_check(base.volume_linear > 0.4 and base.volume_linear < 0.6 and battle.volume_linear > 0.4 and battle.volume_linear < 0.6, "Battle-to-Base crossfade is halfway through after four real-time seconds")
	...
	await _wait(1.0)
	_check(battle.volume_linear > 0.4 and battle.volume_linear < 0.6, "A restarted Battle uses the two-second crossfade")
	Audio.play_battle_music()
	await _wait(1.15)
	_check(battle.get_playback_position() < before and is_equal_approx(battle.volume_linear, 1.0) and base.stream_paused, "Repeated Battle requests preserve playback and the in-progress fade")
	Audio.play_battle_music(true)
	_check(battle.get_playback_position() < 0.1, "A rematch restarts music even when Battle is already selected")
	await _wait(2.15)
	_check(is_equal_approx(battle.volume_linear, 1.0) and is_zero_approx(base.volume_linear), "A rematch fades Battle to full volume")
	Audio.play_base_music()
	await _wait(0.2)
	...
	await _wait(2.15)
	_check(base.stream_paused and is_zero_approx(base.volume_linear) and is_equal_approx(battle.volume_linear, 1.0), "Interrupted crossfades settle on the last requested track")
	Audio.stop_music()
	await _wait(0.1)
	base_position = base.get_playback_position()
	Audio.play_base_music()
	await _wait(0.2)
	_check(not base.stream_paused and base.get_playback_position() > base_position and base.volume_linear < 0.1 and battle.volume_linear > 0.5, "Interrupting a stop resumes Base with the slow crossfade")
	Audio.stop_music()
	await _wait(0.6)
	...
	await _wait(0.6)
	_check(not base.has_stream_playback() and is_equal_approx(battle.volume_linear, 1.0), "Direct Battle entry starts only Battle music")
	Audio.base_music = null
...
func _check_scenes_and_menu() -> void:
	...
	_check(battle_music.get_playback_position() < position and battle_music.volume_linear > 0.2 and battle_music.volume_linear < 0.5, "Entering a new combat scene restarts Battle with a two-second fade")
	var restart: Button
	...
	for path in ["day_summary/day_summary", "victory/victory", "game_over/game_over"]:
		...
		_check(not base_music.stream_paused and base_music.get_playback_position() > position and base_music.volume_linear > 0.04 and base_music.volume_linear < 0.2 and battle_music.volume_linear > 0.8, path + " starts the eight-second return to Base music")
	position = base_music.get_playback_position()

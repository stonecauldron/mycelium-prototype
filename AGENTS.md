...
- Each `EnemyUnitData` authors average STR/DEX/CON/SPD (`stats`), `biomass_reward`, and `show_held_weapon`. Instance stats roll ±1 via `make_stats()`; enemies do **not** use `PowerTier`. Projectiles always come from `combat.projectile_scene` even when held weapon art is hidden.

...
func _run() -> void:
	...
	_check(starter_cues.is_empty(), "Starter confirmation and receipt trigger no SFX")
	_check(GameState.troop.is_seeded(), "Silent confirmation still grants the starter units")
	await _wait()
	await _capture("base")
	await _key(KEY_ESCAPE)
	await _button(menu, "TitleButton")

...
- Each `EnemyUnitData` authors average STR/DEX/CON (`stats`), `composition_cost`, `is_strong`, and `show_held_weapon`. Instance stats roll ±1 via `make_stats()`; enemies do **not** use `PowerTier`. Attack interval is authored on `combat`, not a Stat. Projectiles always come from `combat.projectile_scene` even when held weapon art is hidden.
- Scout UI lives in `scout_bubble/scout_bubble.tscn` (`ScoutBubble`); it fills `GameState.upcoming_enemy_formation`. Roster build reads that array (combat via `BattleLaunch`). Scout reroll costs `BiomassData.reroll_price` (2 biomass + 1 per previous Scout reroll this Day, independent of Shop) and bias-picks a different difficulty by varying the allocated Army budget (scored from actual `composition_cost` totals) — **disabled on elite days**. Scout shows the formation's **Battle reward** (day base × difficulty), not per-kill sums.
- Winning a battle grants a day-scaled **Battle reward**: base `10 + 5 × (day − 1)`, then ±10% from where that army's actual cost sits within its authored Day-budget range (`EnemyComposer.battle_reward_for`). Lump sum on victory; no per-kill biomass.
- Every 5th battle is elite (`GameState.is_elite_day`: days 5 and 10): higher Army-budget range and Strong enemies only, seeded by `GameState.run_seed` + day. No scout reroll; skull hover on the base progression track previews that elite army in the scout bubble.
...
- Day 1 is Solar Sword-only; Rose Thorn/Peashooter unlock on day 2 and Stump on day 4. Days 1–4 allow only Regular enemies; days 5/10 only Strong enemies; days 6–9 at most two distinct Strong types. `EnemyComposer` allocates a Day budget, picks a pattern and eligible types, then fits headcounts to authored costs. Tuning and verification: `.scratch/budgeted-enemy-generation/spec.md`. Run start: blocking seal pick (`SealChoiceDialog`, 3 rolled offers), then War Chamber package pick (`StarterPackages` / `StarterChoiceDialog`) — one dual-trained combo Adult (common tier) plus a hidden untrained Child. Starting biomass is 3. Mid-run seal picks still queue after days 2 / 5 / 8 (reroll allowed at `BiomassData.seal_reroll_price`; opening pick cannot reroll).

...
func _run() -> void:
	...
	_label("Mushrooms — matte art pass", Vector2(210, 32), 1500, 40)
	...
	for adult in [true, false]:
		for column in labels.size():
			...
			var cap_mutation: MutationData = null
			if column == 5:
				cap_mutation = load("res://assets/base/nursery/mutations/cap/inky.tres") as MutationData
			var appearance := UnitAppearance.compose_player(adult, mutation, cap_mutation)
	...
	_label("Matte shading • layered caps and bodies • mutation colors", Vector2(200, 940), 1520, 28)
	for i in 5:
		await get_tree().process_frame
	if "--screenshot" in OS.get_cmdline_user_args():
		await RenderingServer.frame_post_draw
		var output := get_viewport().get_texture().get_image()
		_check(output.save_png("res://.scratch/mushroom-art-pass/preview.png") == OK, "Preview saved")
